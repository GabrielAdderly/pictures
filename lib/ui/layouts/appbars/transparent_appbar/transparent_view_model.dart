import 'package:flutter/foundation.dart';

import 'package:redux/redux.dart';

import 'package:pictures_view/res/typedef.dart';

import 'package:pictures_view/store/application/app_state.dart';
import 'package:pictures_view/store/shared/route_selectors.dart';

class TransparentAppBarViewModel {
  final PopFunction  pop;

  TransparentAppBarViewModel({
    @required this.pop,
  });

  static TransparentAppBarViewModel fromStore(Store<AppState> store) {
    return TransparentAppBarViewModel(
      pop: RouteSelectors.pop(store),
    );
  }
}
